...
otp_enc.c
Author: Ryan Ruiz
Description: Client side program that connects to otp_enc_d. otp_enc sends
plaintext and a key for otp_enc_d to encode and send back ciphertext.
Plaintext and key must have valid characters only containing
A-Z or SPACE only.
Command Line Format: otp_enc plaintext key [portNumber]
...
int main(int argc, char *argv[]) {
   ...
   charsWritten = send(sockfd, "enc", 3, 0);
   if (charsWritten < 0)
   ...
   if(strcmp(confirmation, "success") == 0) {
      ...
      /* If plaintext and key valid receive ciphertext */
      else {
         charsWritten = send(sockfd, "enc", 3, 0);
         if (charsWritten < 0)
            fprintf(stderr,"CLIENT: ERROR writing to socket\n");
         ...
         /* Receive encoded plaintext from server */
      ...
      fprintf(stderr,"ERROR: opt_enc not compatible with program on port %s\n", argv[3]);
      exit(1);
   }
   return 0;
}