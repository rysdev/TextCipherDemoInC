...
otp_enc_d.c
Author: Ryan Ruiz
Description: Server side program that connects to otp_enc. otp_enc_d receives
plaintext and a key from otp_enc. opt_enc_d encodes the plaintext using the key
and sends the coded text back ciphertext to client.
Plaintext and key must have valid characters only containing
A-Z or SPACE only.
Command Line Format: otp_enc_d [portNumber]
...
void handleConnection(int incomingFD) {
   ...
   if(strcmp(userInput, "enc") == 0) {
      ...
      /* Strip off newline char of plainfile for encoding */
      ...
      /* Strip off newline char of key for encoding */
      ...
      /* If key or plaintext valid */
         ...
         memset(userInput, '\0', sizeof(userInput));
         /* Receive confirmation to transmit encoded message */
         charsIn = recv(incomingFD, userInput, sizeof(userInput) - 1, 0);
         if(charsIn < 0)
            fprintf(stderr,"SERVER: ERROR reading from socket\n");

         /* Transmit encoded plaintext */
         if(strcmp(userInput, "enc") == 0) {

            /* Add newline to encoded message before sending to client */
